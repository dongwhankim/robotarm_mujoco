...
CFInite_State_Machine::CFInite_State_Machine()
{
...
void CFInite_State_Machine::timereset(double target_time)
{
    d->time = d->time - target_time;
}
...
void simulate(void)
{
    ...
    FILE* fp = fopen("C:/Users/kist/Desktop/방역로봇_mujoco_simulation", "w");
    FILE* fp2 = fopen("C:/Users/kist/Desktop/방역로봇_mujoco_simulation", "w");
    ...
    while (!settings.exitrequest)
    {
        ...
        if (m)
        {
            ...
            cout << d->time << endl; //시간 체크
            // running
            if (settings.run)
            {
                //cout << d->time << endl; //시간 체크
                CController Control(m->nv, d->qpos, d->qvel);

                Control.Finite_State_Machine(d->time);
                /*
                if (d->time >= 28)  // 1주기 시간마다 재시작
                {
                    d->time = d->time - d->time;
                }
                */
                Control.output_torque_mujoco(d->ctrl);
               // fprintf(fp, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf\n", x_des[1], x_des[2], x_des[3], q_des[1], q_des[2], q_des[3], d->qpos[1], d->qpos[2], d->qpos[3], d->qvel[1], d->qvel[2], d->qvel[3], Control._torque(1), Control._torque(2), Control._torque(3), d->time);// , Trajectory[0]._time_start);
...
int main(int argc, const char** argv)
{
    ...
    init();
    char str[100] = "../mujoco200_win64/model/wow_tablebeside.xml";
    argv[1] = str;
    argc = sizeof(argv);
